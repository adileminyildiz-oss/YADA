...
## 🟢 Dernière mise à jour — Consultation : flèches d'exercice EN DOUBLE retirées (haut de page) → une seule paire (les boutons ronds du bas pilotent l'exercice) — v564
**Quoi :** dans la **Consultation**, il y avait **deux paires de flèches** empilées en haut à gauche — les **flèches « ← Exercice 2026 → » du bandeau de titre** (`.sg-exoarr`) **et** les **boutons ronds du bas** (colonne « Périod… », ajoutés en v563). C'était un **doublon** visuel. À la demande, les **flèches du haut sont retirées** ; il ne reste **qu'une seule paire** — les **boutons ronds** (colonne des périodes) — qui **pilotent désormais l'exercice (année précédente / suivante)** via `exPrecedent()`/`exSuivant()`. Le **libellé « Exercice 2026 » reste** affiché dans le bandeau de titre (informatif, il se met à jour au changement d'année). Les **mois** restent sélectionnables en **cliquant dans la liste des périodes** (01/26 → 12/26).

**Comment — `yada-addon-consult-nav` (édition, precompta + V1) :** (1) ajout `html body:not(#_yz) .sg-app .sg-exonav .sg-exoarr:not(.sg-edlink){display:none !important}` (masque les flèches du haut, garde le libellé `.sg-exolbl` et le lien `.sg-edlink` Éditions) + `ensure()` force `style.display='none'` sur ces flèches (ceinture-bretelles) ; (2) les deux `.yc-navbtn` de la barre `.sg-mois-nav` (colonne Périodes) appellent désormais `exPrecedent()` / `exSuivant()` (au lieu de `sgMoisNav(±1)`), titres « Exercice précédent/suivant » ; le libellé interne redondant est retiré (le titre du haut affiche déjà l'année). `window.sgMoisNav` conservé (inutilisé). `sw.js` yada-v159, badge v564, `version.json` 564, purge `yada-fresh-564`.

**Validé :** `node --check` (precompta 235 / V1 234 scripts, 0 erreur) + `node --check sw.js` OK + filet d'équilibre (`node tests/equilibre-ecritures.mjs` : vente 1200=1200, achat 600=600 ✅) + Playwright (Consultation : **0 flèche d'exercice visible en haut** ; barre `.sg-mois-nav` présente = 2 boutons ronds titrés « Exercice précédent/suivant » `←`/`→` ; libellé « Exercice 2026 » conservé en haut ; 0 pageerror) + capture. Badge → **v564**. *(Prochaines étapes convenues : retrait de l'ancien système de notification + nouvelles notifications d'après maquettes + nouveau bouton de téléchargement d'après maquettes.)*
