#!/usr/bin/env bash
# Vérifie le Lot L3 (GED, OCR & réception) sur cluster PostgreSQL local.
# Socle L0→L3 (migrations 0001→0005). Prouve :
#  - extraction facture (pure, sans réseau)
#  - dépôt→lecture→pré-remplissage, doublon, fiche inconnu→tiers, 2 temps, coffre-fort (live).
set -euo pipefail
cd "$(dirname "$0")/.."
...
export JWT_SECRET="${JWT_SECRET:-l3-verify-secret}"
...
echo "═══ 2. Migrations (CORE→Réception) ═══"
node scripts/migrate.mjs

echo "═══ 3. Build ═══"
npm run build >/dev/null

echo "═══ 4. Extraction facture (pure) ═══"
node scripts/extract.test.mjs

echo "═══ 5. Réception / doublon / fiche / coffre-fort (live) ═══"
node dist/main.js >/tmp/yada-api.log 2>&1 & API_PID=$!; wait_api
node scripts/l3-check.mjs

echo ""
echo "╔════════════════════════════════════════════════╗"
echo "║  L3 — CRITÈRE DE SORTIE VALIDÉ ✓                ║"
echo "║  dépôt→lu→validé (2 temps) · doublon · fiche   ║"
echo "║  → tiers · coffre-fort cherchable              ║"
echo "╚════════════════════════════════════════════════╝"
